...
== Usage
To apply the JavaCC Gradle Plugin to your projects, add the following in your `build.gradle` script:
...
javacc {
    ...
    configs {
        template { // That name ('template') serves as a configuration name, for example in the output folder path
            // For available properties, see column "Property" below
...
// If you want a specific version of JavaCC
dependencies {
...
Then invoke as `gradle javacc`. That generates files under `build/generated/javacc/template/your/package/name/`, where `your/package/name` comes from `packageName` above, with dots replaced by a slash.
